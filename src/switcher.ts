import { MANAGED_ENV_KEYS, PROVIDERS, getAllProviders, getAllManagedEnvKeys, type ProviderDefinition } from "./providers.js";
import {
  readConfig,
  writeConfig,
  type SwitchConfig,
...
async function backupNativeEnv(
  ...
  managedKeys: readonly string[] = MANAGED_ENV_KEYS,
...
export async function switchProvider(
  ...
  if (provider.id === "claude") {
    ...
    const newCustomKeys = Object.keys(providerEnv).filter(
    ...
    if (newCustomKeys.length > 0) {
      ...
      for (const k of newCustomKeys) existing.add(k);
