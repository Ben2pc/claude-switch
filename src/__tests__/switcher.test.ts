...
import { PROVIDERS, MANAGED_ENV_KEYS, buildCustomProviderDefinition, type CustomProviderConfig } from "../providers.js";
import {
...
describe("switchProvider", () => {
  ...
  function setupNativeState(env?: Record<string, string | number>) {
    mockReadConfig.mockResolvedValue({});
  ...
  describe("native env backup/restore", () => {
    it("backs up native env keys when switching FROM Claude native", async () => {
      ...
      const backupCall = mockWriteConfig.mock.calls[0][0] as SwitchConfig;
    ...
    it("does NOT backup when switching between two third-party providers", async () => {
      ...
      for (const call of mockWriteConfig.mock.calls) {
        const config = call[0] as SwitchConfig;
    ...
    it("clears nativeEnvBackup from config after restore", async () => {
      ...
      const configCall = mockWriteConfig.mock.calls.find(
        (call) => (call[0] as SwitchConfig).nativeEnvBackup === undefined,
      );
  ...
  describe("MCP cleanup on native return", () => {
    it("cleans up managed MCPs when switching to Claude native", async () => {
      ...
      // User MCP preserved
      const writtenServers = mockWriteMcpServers.mock.calls[0][0] as Record<string, unknown>;
  ...
  describe("logging", () => {
    ...
    it("redacts API key in log, keeping first 4 and last 4 chars", async () => {
      ...
      const detail = logCall![1] as Record<string, unknown>;
      const envWritten = detail.envWritten as Record<string, string>;
...
describe("cleanupManagedMcps", () => {
  ...
  it("removes only MCPs listed in enabledMcps from mcpServers", async () => {
    ...
    const written = mockWriteMcpServers.mock.calls[0][0] as Record<string, unknown>;
  ...
  it("clears enabledMcps from config after cleanup", async () => {
    ...
    const configCall = mockWriteConfig.mock.calls[0][0] as SwitchConfig;
...
describe("custom provider switching", () => {
  const customCp: CustomProviderConfig = {
    id: "my-proxy",
  ...
  it("detectActiveProviderFromSettings recognizes custom provider", () => {
    const allProviders = [...PROVIDERS, customProvider];
    const result = detectActiveProviderFromSettings(
      { env: { ANTHROPIC_BASE_URL: "https://my-proxy.example.com/v1" } },
      allProviders,
    );
  ...
  it("saves activeProviderId on switch, clears on return to claude", async () => {
    ...
    const switchCall = mockWriteConfig.mock.calls.find(
      (call) => (call[0] as SwitchConfig).activeProviderId === "my-proxy",
    );
    ...
    const clearCall = mockWriteConfig.mock.calls.find(
      (call) => (call[0] as SwitchConfig).activeProviderId === undefined,
    );
  ...
  it("backs up custom managed keys when leaving native", async () => {
    ...
    const backupCall = mockWriteConfig.mock.calls.find(
      (call) => (call[0] as SwitchConfig).nativeEnvBackup?.CUSTOM_TIMEOUT === "9999",
    );
