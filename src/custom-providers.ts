 ...
 */
function readMultiLineJson(message: string): Promise<string | null> {
 ...
 */
function validateId(id: string, existingIds: Set<string>): string | true {
  if (!id.trim()) return "ID cannot be empty";
  if (/\s/.test(id)) return "ID cannot contain spaces";
  if (RESERVED_IDS.has(id)) return `"${id}" conflicts with a built-in provider or reserved word`;
 ...
 * Validate a base URL.
 */
function validateBaseUrl(url: string): string | true {
 ...
 */
async function promptModels(): Promise<ProviderModel[] | null> {
  ...
  while (true) {
    try {
      ...
      if (isDefault) {
        for (const m of models) m.default = undefined;
        model.default = true;
      }
 ...
 */
async function addCustomProviderWizard(config: SwitchConfig): Promise<SwitchConfig | null> {
  ...
  try {
    ...
    if (method === "json") {
      ...
      if (promptedKey === null) return null;
      apiKey = promptedKey;
    }
 ...
 */
async function editCustomProviderWizard(
  config: SwitchConfig,
  cp: CustomProviderConfig,
): Promise<SwitchConfig | null> {
  try {
    ...
    const existingIds = new Set(
      (config.customProviders ?? []).filter((p) => p.id !== cp.id).map((p) => p.id),
    );

    let updates: Partial<CustomProviderConfig> = {};
 ...
 */
async function deleteCustomProviderFlow(
  config: SwitchConfig,
  cp: CustomProviderConfig,
): Promise<SwitchConfig | null> {
  try {
 ...
 */
export async function manageCustomProviders(config: SwitchConfig): Promise<void> {
