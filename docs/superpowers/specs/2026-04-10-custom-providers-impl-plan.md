...
- `buildCustomProviderDefinition(def)` 正确转换自定义 provider 为 `ProviderDefinition`：
  - 无 `envVars` 时 `buildEnv` 生成默认三件套
  - 有 `envVars` 时 `{{API_KEY}}` 和 `{{MODEL}}` 占位符被替换
  - 无 `models` 时 `models` 为 `[]`
- `getAllProviders(config)` 合并内置 + 自定义，ID 冲突时打 warning 并跳过自定义
- `getAllManagedEnvKeys(config)` 动态合并自定义 provider 的 env keys

**改动** `src/providers.ts`：
- 新增 `CustomProviderConfig` 接口：`{ id, displayName, baseUrl, models?, envVars? }`
...
- `addCustomProviderWizard(config)` — 添加向导：
  ...
  4. Models（循环添加 name/displayName/description/default）
  5. Env Vars（三选一：默认 / key-value 逐条 / 粘贴 JSON）
  6. 确认保存
...
**问题**：`detectActiveProviderFromSettings` 通过 `settings.env.ANTHROPIC_BASE_URL` 匹配 `provider.baseUrl` 来识别当前 provider。如果自定义 provider 的 `envVars` 没写 `ANTHROPIC_BASE_URL`、或值和 `baseUrl` 不同，会导致识别失败和 native 备份逻辑误触发。

**修正**：
- `buildCustomProviderDefinition` 中，无论用户是否显式写了 `envVars`，**都强制保证** `envVars.ANTHROPIC_BASE_URL === baseUrl`
- 默认模板已满足；显式 `envVars` 场景下，若用户没写 `ANTHROPIC_BASE_URL` 则自动注入，若写了但值不等于 `baseUrl` 则以 `baseUrl` 为准并打 warning
...
- 显式 `envVars` 中包含 `{{MODEL}}` 但无 models 时：TUI 添加向导给 warning 提示

### MEDIUM: envVars schema 校验

**修正**：
- `getAllProviders` 加载自定义 provider 时做基本校验：
  - `envVars` 所有 value 必须是 string 类型，否则跳过该 provider 并打 warning
  - `id` 不能为空字符串或包含空白字符
